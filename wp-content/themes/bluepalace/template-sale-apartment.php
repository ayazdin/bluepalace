...
if($ct_header_listing_search != 'yes') {
	...
	echo ' ';
	if($total_results != '1') { esc_html_e($_GET['key'].' '.$_GET['property_type'], 'contempo'); } else { esc_html_e('listing found', 'contempo'); }
	echo '</h5>';
	...
	echo  '<div class="breadcrumb breadcrumbs ct-breadcrumbs right muted">';
		echo '<div class="breadcrumb-trail">';
			echo '<span class="trail-before"><span class="breadcrumb-title"></span></span>';
			echo '<a id="bread-home" href="' . home_url() . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" rel="home" class="trail-begin">'.__( 'Home', 'contempo' ).'</a>';
...
if($ct_disable_listing_search_results_adv_search == 'no' &&  $ct_search_results_layout != 'sidebyside') {
			...
			<div class="container">
	<h3 class=" pagetitle contacttitle"><?php echo 'Properties for '. esc_html($_GET['key'])?></h3>
				<!-- Listing Results -->
