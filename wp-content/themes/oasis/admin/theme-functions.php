...
//add active class in li with class current-menu-item
...
if(!function_exists('ct_init_scripts')) {
	function ct_init_scripts() {
		...
		wp_enqueue_style('main', get_template_directory_uri() . '/css/style.css', '', '', 'screen, projection');
		wp_enqueue_style('new custom', get_template_directory_uri() . '/css/newcustom.css', '', '', 'screen, projection');
		...
		//for new theme
		wp_enqueue_script('core', get_template_directory_uri() . '/js/core.min.js', array('jquery'), '1.0', false );
		wp_enqueue_script('mainscripts', get_template_directory_uri() . '/js/script.js', array('jquery'), '1.0', false );
	...
	if(!function_exists('ct_search_form_select')) {
		function ct_search_form_select($name, $taxonomy_name = null) {
			...
			if(!$taxonomy_name) {
				...
				if($tax_name_stripped == 'location') {
					$tax_name = __('Search by location', 'contempo');
				}
				elseif($tax_name_stripped == 'property type') {
...
if(!function_exists('ct_breadcrumbs')) {
	function ct_breadcrumbs( $args = array() ) {
		...
		if ( !is_front_page() && $show_home )
			$trail[] = '<li><a id="bread-home" href="' . home_url() . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" rel="home" >' . $show_home . '</a></li>';
...
//				$breadcrumb .= '<span class="trail-before">' . $before . '</span> ';
			...
			if ( !empty( $trail['trail_end'] ) )
				$trail['trail_end'] = '<li class="active">' . $trail['trail_end'] . '</li>';
